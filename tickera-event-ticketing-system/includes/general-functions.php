...
function tc_quantity_selector($ticket_id) {
    $quantity = apply_filters('tc_quantity_selector_quantity', 10);
...
function tc_get_tickets_count_sold($ticket_id) {
    ...
    foreach ($sold_records as $sold_record) {
        if (get_post_status($sold_record->post_parent) !== 'trash') {
...
//Send e-mail to the client when order is placed / pending
        if ((isset($tc_email_settings['client_send_placed_message']) && $tc_email_settings['client_send_placed_message'] == 'yes')) {//default is no
            ...
            $subject = isset($tc_email_settings['client_order_placed_subject']) ? $tc_email_settings['client_order_placed_subject'] : __('Order Placed', 'tc');
...
function tc_get_ticket_download_link($field_name, $field_id, $ticket_id) {
    ...
    if ($use_order_details_pretty_links == 'yes') {
        ...
        echo apply_filters('tc_download_ticket_url_front_link', '<a href="' . $download_url . '">' . __('Download', 'tc') . '</a>', $ticket_id, $ticket->details->post_parent);
    } else {
...
function tc_get_ticket_checkin_availability_dates($field_name = '', $post_id = '') {
    ...
    <label><input type="radio" name="_ticket_checkin_availability_post_meta" value="range" <?php checked($currently_selected, 'range', true); ?> /><?php _e('During selected date range', 'tc'); ?></label><br />
    <br />
    <label>
...
function tc_get_ticket_types($field_name = '', $post_id = '') {
    ...
    ?>
    <select name="<?php echo esc_attr($field_name); ?>_post_meta[]" multiple="true">
